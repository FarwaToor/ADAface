...
from model_config import ADA_MODEL_PATH
from face_alignment import align

SUPPORTED_ARCHITECTURES = {'ir_101'}

def load_pretrained_model(architecture='ir_101'):
    assert architecture in SUPPORTED_ARCHITECTURES
    model = net.build_model(architecture)
    print(f"Loading model from {ADA_MODEL_PATH}")
    statedict = torch.load(ADA_MODEL_PATH, map_location='cpu', weights_only=True)['state_dict']
